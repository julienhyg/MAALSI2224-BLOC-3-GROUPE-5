...
Ce *PAQ* reprend divers points de ce rapport. Nous l'avons volontairement alégé afin de ne pas surcharger ce rapport, tout en synthétisant les idées. Pour l'instant nous nous concentrons sur les aspects fonctionnels, les détails techniques des tests seront établis par la suite.
...
Bien qu'avec avec une méthode de travail *AGILE*, comme *SCRUM*, la notion de chef de projet n'existe pas, nous définissons un responsable de la coordination des activités que nous appelerons **Service Owner**.  
...
Dans le cadre du développement de l'application web permettant la vente en ligne de *Breizhsport*, nous souhaitons mettre en place les activités suivantes afin de garantir la qualité de la solution pour qu'elle répondre au besoin de l'entreprise.

 -  **Vérification des exigences** : nous verifions les exigences imposées pour l'application, afin de vérifier qu'elles sont complètes, cohérentes, et correctes.
...
Des détails des tests, et de certaines vérification comme le code, les outils utilisés, sont décrits par la suite dans ce rapport. 
